...
urls <- paste0("https://www.dice.com/jobs/q-data_science-startPage-",1:70, "-jobs?searchid=9816474276503&stst=")
keywords <- c('\\bHadoop\\b', '\\bSpark\\b', 
                 '\\bR\\b', '\\bSAS\\b', '\\bStata\\b', 
                 '\\bJava\\b','\\bC+\\b', '\\bPerl\\b', '\\bPython\\b',
                 '\\bSQL\\b', '\\bNoSQL\\b',
...
for(i in seq_along(urls)) {
  url <- urls[i]
   session <- html_session(url)
   job.names <- read_html(url) %>% html_nodes(".loggedInVisited span") %>% html_text()
  for ( j in job.names){
